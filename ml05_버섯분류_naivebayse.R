...
# 1. 데이터 준비
mushroom = read.csv('mlwr/mushrooms.csv')

# 2. 데이터 확인, 전처리
str(mushroom)
summary(mushroom)
# veil_type 변수(특징)는 1개 팩터 변수로 분류에 필요 없는 데이터이다
# 모든 관찰 값에서 항상 같은 값('partial')
# 버섯을 분류할때 사용되지 않는 변수 -> 데이터 프레임에서 제거
...
# 5. 모델 향상 - 라플라스 추정량 변경
classifier2 = naiveBayes(mushroom_train, train_label,
                        laplace = 0.1)
mushroom_predict2 = predict(classifier2, mushroom_test)

CrossTable(x = test_label, y = mushroom_predict2, prop.chisq = F)
