...
exam[1, 5] # 1번 학생(row=1)의 과학 범수(column=5)
# 행 번호 또는 열 번호를 생략하면 행/열을 전체 출력
exam[1, ] # 컬럼 인텍스를 생략하면 해당 행의 모든 컬럼을 출력
exam[, 2] # 행 인덱스를 생략하면 해당 컬럼의 모든 행을 출력
# 모든 학생읭 영어 점수를 출력
exam[, 4] # (oracle: select english)
# 5번 학생의 모든 정보를 출력
exam[5, ] # (oracle: select * from exam where id = 5)
...
# mpg 데이터 프레임에서 
# 시내주행 연비가 평균이상이고, 고속도로 연비도 평균 이상인 자동차들의
# model, cty, hwy를 출력
mpg[mpg$cty >= mean(mpg$cty) & mpg$hwy >= mean(mpg$hwy) , c('model', 'cty', 'hwy')]
dim(mpg[mpg$cty >= mean(mpg$cty) & mpg$hwy >= mean(mpg$hwy) , c('model', 'cty', 'hwy')])
